...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  var p = new Poller(check.url, this.config.timeout, function(err, time, version) {
    //todo: when new check type is created, insert conditional here. 
...
Monitor.prototype.createPing = function(check, error, timestamp, time, callback) {
  ...
  var postData = 'checkId=' + check._id +  '&version=' +  check.newVersion + '&status=' + status + '&timestamp=' + timestamp + '&time=' + time + '&name=' + this.config.name + '&error=' + (error ? error.message : '');
