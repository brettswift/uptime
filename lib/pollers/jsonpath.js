 ...
 * @api   public
 */
function JsonPathPoller(check, timeout, callback) {
  JsonPathPoller.super_.call(this, check, timeout, callback);

}

util.inherits(JsonPathPoller, BasePoller);

JsonPathPoller.prototype.initialize = function() {
 ...
 * @api   public
 */
JsonPathPoller.prototype.setUserAgent = function(userAgent) {
 ...
 * @api   public
 */
JsonPathPoller.prototype.poll = function() {
  JsonPathPoller.super_.prototype.poll.call(this);
 ...
 * @api   private
 */
JsonPathPoller.prototype.onResponseCallback = function(res) {
  ...
  if (statusCode == 200) {
    ...
    res.on('end', function() {
      ...
      contentParser.getXmlValue(body, poller.check.JsonPathPoller, function(err, version){
        var version = version;
        poller.callback(undefined, poller.getTime(), version);
      });
 ...
 * @api   private
 */
JsonPathPoller.prototype.timeoutReached = function() {
  JsonPathPoller.super_.prototype.timeoutReached.call(this);
  this.request.removeAllListeners('error');
  this.request.on('error', function() { /* swallow error */ });
  this.request.abort();
};

module.exports = JsonPathPoller;
